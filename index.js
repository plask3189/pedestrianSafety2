...
// section.addEventListener("mouseover", setHoverFlag);

// section.addEventListener("click", adjustHeight);
// section.addEventListener("mouseleave", revertHeight);
// document.addEventListener("DOMContentLoaded", () => {
//   section.addEventListener("mouseleave", (event) => {
//     event.stopPropagation();
//     section.classList.toggle("active");
...
function getPoints(lat=0, lng=0) {
  let maps_list;
  maps_list= [
  ...
  if (lat !== 0 && lng !== 0) {
    ...
    maps_list.push(newPoint);
  }
  
  if (heatmap) {
    heatmap.setData(maps_list); 
  } else {
